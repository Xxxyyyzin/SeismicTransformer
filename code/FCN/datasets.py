...
class SeismicBinaryDBDataset(SeismicDataset):
    ...
    def __init__(self,
        ...
        if file_size < total_size: raise Exception(
            "ERROR: file size < expected size: %s (%i < %i)" % (self.c.DATA_PATH, file_size, total_size))
        if file_size > total_size: print(
            "WARNING: file size > expected size: %s (%i != %i)" % (self.c.DATA_PATH, file_size, total_size))
    ...
    def __getitem__(self, i):
        ...
        offset += delta;
        delta = np.prod(self.c.GATHER_SHAPE)
        gather = array[offset:offset + delta]
        offset += delta;
        delta = np.prod(self.c.SOURCE_SHAPE)
