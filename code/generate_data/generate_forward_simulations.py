...
QC_FREQ = 10 # when to output full wavefield or not (I/O heavy)
n_processes = 6
...
def generate_example(ivel):
    ...
    if SIM_NUM % QC_FREQ == 0: OUTPUT_WAVEFIELD = 0
    else: OUTPUT_WAVEFIELD = 1# whether to output wavefield (I/O heavy!)
...
def worker_function(taskQ, resultQ):
    ...
    while True:
        try: ivel = taskQ.get(block=True, timeout=10)
