...
VEL_RUN = "fault"        #"fault_generalisation"
SIM_RUN = "fault_2ms_r"     #"fault_2ms_r_generalisation"
...
QC_FREQ = 200  # when to output full wavefield or not (I/O heavy)
n_processes = 6
...
if RANDOMSOURCE:  # 3 sources per velocity model
    ...
    source_is = np.concatenate(
        [(NX // 2) * np.ones((N_VELS, 1, 1), dtype=int), SOURCE_Yi * np.ones((N_VELS, 1, 1), dtype=int)], axis=2)

receiver_is = np.array(
    [int(np.floor((DELTAX * NX / 2. - (N_REC - 1) * DELTAREC / 2. + i * DELTAREC) / DELTAX)) for i in range(N_REC)])
...
def generate_example(ivel):
    ...
    if SIM_NUM % QC_FREQ == 0:
        OUTPUT_WAVEFIELD = 0
    else:
        OUTPUT_WAVEFIELD = 1  # whether to output wavefield (I/O heavy!)
    ...
    for isource, source_i in enumerate(source_is[SIM_NUM]):
        ...
        with open(RECEIVER_FILE, 'w') as f:
            f.write("%i\n" % (N_REC))
            for rec_i in receiver_is: f.write(
        ...
        with open(file, 'rb') as f:
            ...
            for irec in np.arange(N_REC): gather[irec, :] = np.fromfile(f, dtype=np.float64, count=NSTEPS).astype(
                np.float32)
        ...
        if OUTPUT_WAVEFIELD:
            ...
            files = [TEMP_OUT_SIM_DIR + "wavefield_%.8i_%.8i.bin" % (SIM_NUM, i + 1) for i in
                     range(NSTEPS)]  # SEISMIC CPML uses indices starting at 1
            for i in range(NSTEPS):
                # Read each binary wavefield file (MUCH QUICKER THAN READING TEXT FILES, beacause its directed)
                with open(files[i], 'rb') as f:
                    # Note SEISMIC_CPML double precision saved to 64 bit floats (!) we DOWNSAMPLE to 32 bit floats
                    # count = number of items (==np.float64 values) to process)
                    for iz in np.arange(NY): wavefields[i, :, iz] = np.fromfile(f, dtype=np.float64, count=NX).astype(
                        np.float32)
...
def generate_simulation():
    ...
    shutil.copyfile('generate_simulation.py', OUT_SIM_DIR + 'generate_forward_simulations_%s.py' % (SIM_RUN))
