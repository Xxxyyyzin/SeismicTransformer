"""
This script is :Transformer模型下绘制不同震源位置下的预测图，含差分图
"""
import numpy as np
...
for i in range(2):
    ...
    # velocity
    plt.subplot2grid((2, 32), (i, 0), colspan=4)
    ...
    for source_i in range(3):
        ...
        # gather_ground_truth
        plt.subplot2grid((2, 30), (i, 4 + 7 * source_i), colspan=2)
        ...
        # gather_ground_truth
        plt.subplot2grid((2, 30), (i, 4 + 7 * source_i + 2), colspan=2)
    ...
    if i % 2 == 1:
        ax = f.add_axes([0.75, 0.68, 0.01, 0.20])
        cb = f.colorbar(im0, cax=ax)
        cb.ax.set_ylabel('Velocity ($\mathrm{ms}^{-1}$)')

        ax = f.add_axes([0.75, 0.267, 0.01, 0.20])
        cb = f.colorbar(im1, cax=ax, aspect=0.01)
        cb.ax.set_ylabel('Pressure (arb)')

plt.savefig("../report_plots/different_shot20.21.pdf", bbox_inches='tight', pad_inches=0.01, dpi=300)
plt.show()