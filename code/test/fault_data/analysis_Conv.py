...
def load_model_Conv(MODEL_LOAD_RUN, rootdir="server/", optimizer="Adam", nth_model=0, verbose=False):
...
def load_testdataset_Conv(DATA_PATH,
                     N_EXAMPLES,
                     c_dict,
...
def plot_result_Conv(inputs_array, outputs_array, labels_array, sample_batch=None, title="CONV\n100000th",ib=0, isource=0,
    ...
    plt.imshow((t_gain * (labels_array - outputs_array))[ib, isource, :, :].T, aspect=aspect, cmap="Greys", vmin=gmin,
               vmax=gmax)
...
def plot_result_withoutDIFF_Conv(inputs_array, outputs_array, labels_array, sample_batch=None,title="CONV\n100000th", ib=0, isource=0,
                            aspect=0.2, T_GAIN=2.5, vmin=-1, vmax=1, gmin=-1, gmax=1):
    "Plot a network prediction, compare to ground truth and input"
    f = plt.figure(figsize=(12, 5))
